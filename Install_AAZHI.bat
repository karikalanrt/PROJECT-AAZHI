...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$d = [Environment]::GetFolderPath('Desktop'); $w = New-Object -ComObject WScript.Shell; $s = $w.CreateShortcut((Join-Path $d 'AAZHI Satellite Intelligence.lnk')); $s.TargetPath = '%TARGET_DIR%\Launch_AAZHI.bat'; $s.WorkingDirectory = '%TARGET_DIR%'; $s.IconLocation = 'shell32.dll,14'; $s.Description = 'AAZHI Satellite Intelligence Command'; $s.Save()"
